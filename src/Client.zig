...
pub const Client = struct {
    ...
    pub fn init(
        ...
        const init_message: Message.ClientMessage = msg: {
            if (custom_resolution) |cr| {
                ...
                break :msg .init(framebuffer_id, display_type);
            }
        };

        socket_writer.writeStruct(init_message, .little) catch {
            return error.ClientWriteError;
        };
        socket_writer.flush() catch {
            return error.ClientWriteError;
        };

        const server_response = socket_reader.takeStruct(Message.ServerMessage, .little) catch {
            return error.ClientReadError;
        };
    ...
    pub fn fullRefresh(self: *Client, io: std.Io) Error!void {
        try self.send(io, .full_refresh);
    }

    /// Set the display refresh mode.
    pub fn setRefreshMode(self: *Client, io: std.Io, mode: RefreshMode) Error!void {
        try self.send(io, .refreshMode(mode));

        self.refresh_mode = mode;
    }

    /// Reset the display to default refresh mode.
    pub fn resetRefreshMode(self: *Client, io: std.Io) Error!void {
    ...
    pub fn pollServerPacket(self: *Client, io: std.Io) Error!Message.ServerMessage {
        var read_buf: [@sizeOf(Message.ServerMessage)]u8 = undefined;
        var socket_r = self.socket.reader(io, &read_buf);
        const socket_reader = &socket_r.interface;

        return socket_reader.takeStruct(Message.ServerMessage, .little) catch {
            return error.ClientReadError;
        };
    }

    fn send(self: *Client, io: std.Io, msg: Message.ClientMessage) Error!void {
        var write_buf: [@sizeOf(Message.ClientMessage)]u8 = undefined;

        var socket_w = self.socket.writer(io, &write_buf);
        const socket_writer = &socket_w.interface;

        socket_writer.writeStruct(msg, .little) catch {
            return error.ClientWriteError;
        };
        socket_writer.flush() catch {
            return error.ClientWriteError;
...
pub fn getIDFromAppLoad(env: std.process.Environ) error{NoAppload}!FBKey {
    const key = env.getPosix("QTFB_KEY") orelse {
        return error.NoAppload;
    };

    return std.fmt.parseInt(i32, key, 10) catch {
        return error.NoAppload;
    };
}
